...
class APISession(APIRequest):
    ...
    def __init__(
        ...
        """
        logger.info(
            "mistapi:init:package version %s",
            __version__
        )
    ...
    def _load_env(self, env_file=None) -> None:
        ...
        if env_file:
            ...
            logger.debug(
                "apisession:_load_env:loading settings from %s", 
                env_file
            )
    ...
    def set_cloud(self, cloud_uri: str) -> None:
        ...
        if self._cloud_uri:
            logger.debug(
                "apisession:set_cloud:Mist Cloud configured to %s",
                self._cloud_uri
            )
            console.debug(f"Mist Cloud configured to {self._cloud_uri}")
        else:
            logger.error(
                "apisession:set_cloud: %s is not valid",
                cloud_uri
            )
        ...
        if resp == "q":
            ...
            try:
                ...
                if resp_num >= 0 and resp_num < i:
                    ...
                    logger.error(
                        "apisession:select_cloud:%s is not a valid input",
                        resp
                    )
                    self.select_cloud()
            except:
    ...
    def set_email(self, email: str = None) -> None:
        ...
        logger.info(
            "apisession:set_email:email configured to %s",
            self.email
        )
    ...
    def set_api_token(self, apitoken: str) -> None:
        ...
        logger.info(
            "apisession:set_api_token:found %s API Tokens",
            len(apitokens_out)
        )
    ...
    def _get_api_token_data(self, apitoken) -> (str, list):
        ...
        try:
            ...
            logger.critical(
                "apirequest:mist_post:Connection Error: %s",
                connexion_error
            )
    ...
    def _process_login(self, retry: bool = True) -> None:
        ...
        if resp.status_code == 200:
            ...
            logger.error(
                "apisession:_process_login:authentication failed:%s",
                error
            )
    ...
    def login_with_return(
        ...
        if self._apitoken:
            ...
            uri = "/api/v1/self"
            logger.info(
                'apisession:login_with_return: sending GET request to "%s"',
                uri
            )
        ...
        if resp.status_code == 200 and not resp.data.get("two_factor_required", False):
            ...
            logger.error(
                "apisession:login_with_return:access denied: %s",
                resp.data
            )
    ...
    def get_authentication_status(self) -> bool:
        ...
        """
        logger.debug(
            "apisession:get_authentication_status:return %s",
            self._authenticated
        )

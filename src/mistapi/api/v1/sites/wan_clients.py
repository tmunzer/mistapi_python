...
def countSiteWanClients(
    ...
    distinct : str{'hostname', 'ip', 'mac', 'mfg'}, default: mac
      Field used to group this count response. enum: `hostname`, `ip`, `mac`, `mfg`
    start : str
...
def searchSiteWanClients(
    ...
    mfg : str
      Filter results by manufacturer
    network : str
      Partial / full Name of the network the client is/was connected to. Use `prefix*` for prefix search or `*substring*` for contains search (e.g. `my-corp*` and `*corp*` match `my-corp-network`). Suffix-only wildcards (e.g. `*corp`) are not supported
    limit : int, default: 100
      Maximum number of results to return per page
    start : str
      Lower bound of the time range, as an epoch timestamp in seconds or a relative value such as `-1d` or `-1w`

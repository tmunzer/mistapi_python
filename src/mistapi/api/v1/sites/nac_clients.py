...
def countSiteNacClients(
    ...
    distinct : str{'ap', 'auth_type', 'device_mac', 'edr_managed', 'edr_provider', 'edr_status', 'family', 'hostname', 'idp_id', 'mfg', 'mdm_compliance', 'mdm_managed', 'mdm_provider', 'model', 'mxedge_id', 'nacrule_matched', 'nacrule_name', 'nacrule_id', 'nas_ip', 'nas_vendor', 'os', 'ssid', 'status', 'type', 'usermac_label', 'username', 'vlan'}, default: type
      Field used to group this count response. enum: `ap`, `auth_type`, `device_mac`, `edr_managed`, `edr_provider`, `edr_status`, `family`, `hostname`, `idp_id`, `mfg`, `mdm_compliance`, `mdm_managed`, `mdm_provider`, `model`, `mxedge_id`, `nacrule_matched`, `nacrule_name`, `nacrule_id`, `nas_ip`, `nas_vendor`, `os`, `ssid`, `status`, `type`, `usermac_label`, `username`, `vlan`
    last_nacrule_id : str
...
def countSiteNacClientEvents(
    ...
    type : str
      See [List Device Events Definitions](/#operations/listNacEventsDefinitions)
    start : str
      Lower bound of the time range, as an epoch timestamp in seconds or a relative value such as `-1d` or `-1w`
    end : str
      Upper bound of the time range, as an epoch timestamp in seconds or a relative value such as `-1d`, `-2h`, or `now`
...
def searchSiteNacClientEvents(
    ...
    random_mac : bool
      Filter results by whether the client is using a randomized MAC address
    mac : str
      Filter results by MAC address
    usermac_label : str
      Labels derived from usermac entry
    text : str
      Partial / full MAC address, username, device_mac or ap. Use `prefix*` for prefix search or `*substring*` for contains search (e.g. `aabbcc*` and `*bbcc*` match `aabbccddeeff`). Suffix-only wildcards (e.g. `*bccddeeff`) are not supported
    nas_ip : str
...
def searchSiteNacClients(
    ...
    cert_expiry_duration : str
      Filter by certificate expiry within a specific duration from now (e.g., "7d" for 7 days, "1m" for 1 month)
    edr_managed : bool
    ...
    family : str
      Partial / full Client family (e.g. "Phone/Tablet/Wearable", "Access Point"). Use `prefix*` for prefix search or `*substring*` for contains search (e.g. `Surveillance*` and `*urveillance*` match `Surveillance Camera`). Suffix-only wildcards (e.g. `*Camera`) are not supported
    hostname : str
    ...
    text : str
      partial / full MAC address, last_username, device_mac, nas_ip. Use `prefix*` for prefix search or `*substring*` for contains search (e.g. `aabbcc*` and `*bbcc*` match `aabbccddeeff`). Suffix-only wildcards (e.g. `*bccddeeff`) are not supported.
    type : str
      Client type i.e. "wireless", "wired" etc.
    usermac_label : list
      Labels derived from usermac entry
...
def sendSiteNacClientCoA(
    mist_session: _APISession, site_id: str, client_mac: str, body: dict | list
) -> _APIResponse:
    """
    API doc: https://www.juniper.net/documentation/us/en/software/mist/api/http/api/sites/clients/nac/send-site-nac-client-co-a
    ...
    uri = f"/api/v1/sites/{site_id}/nac_clients/{client_mac}/coa"

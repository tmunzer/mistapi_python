...
def get_by_id(mist_session:Session, org_id, network_id):
    uri = f"/api/v1/orgs/{org_id}/networks/{network_id}" 
    resp = mist_session.mist_get(uri)
    return resp

def update(mist_session:Session, org_id, network_id, settings):
    uri = f"/api/v1/orgs/{org_id}/networks/{network_id}" 
    body = settings
    resp = mist_session.mist_put(uri, body=body)
    return resp

def delete(mist_session:Session, org_id, network_id):
    uri = f"/api/v1/orgs/{org_id}/networks/{network_id}" 

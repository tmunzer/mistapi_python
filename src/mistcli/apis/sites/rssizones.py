...
def create(mist_session:Session, site_id, rssizone_settings):
    uri = f"/api/v1/sites/{site_id}/rssizones"
    body = rssizone_settings
    resp = mist_session.mist_post(uri, body=body)
    return resp

def update(mist_session:Session, site_id, rssizone_id, body={}):
    uri = f"/api/v1/sites/{site_id}/rssizones/{rssizone_id}"
    resp = mist_session.mist_put(uri, body=body)
    return resp
    
def delete(mist_session:Session, site_id, rssizone_id):
    uri = f"/api/v1/sites/{site_id}/ssizones/{rssizone_id}"

...
def create(mist_session:Session, site_id, wxtag_settings):
    uri = f"/api/v1/sites/{site_id}/wxtags"
    body = wxtag_settings
    resp = mist_session.mist_ost(uri, body=body)
    return resp

def update(mist_session:Session, site_id, wxtag_id, body={}):
    uri = f"/api/v1/sites/{site_id}/wxtags/{wxtag_id}"
    resp = mist_session.mist_ut(uri, body=body)
    return resp
    
def delete(mist_session:Session, site_id, wxtag_id):
    uri = f"/api/v1/sites/{site_id}/wxtags/{wxtag_id}"
